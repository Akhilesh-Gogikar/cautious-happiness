...
import { AlpacaDashboard } from '@/components/dashboard/alpaca-dashboard';
import { Activity, Globe, ShieldCheck, Cpu, Zap, TrendingUp } from 'lucide-react';
...
function ViewPlaceholder({ title }: { title: string }) {
    return (
        <div className="flex flex-col items-center justify-center h-[70vh] space-y-8 animate-fade-in text-center relative">
            <div className="absolute inset-0 bg-primary/5 blur-[120px] rounded-full animate-pulse-glow pointer-events-none" />
            <div className="w-24 h-24 rounded-sm bg-white/[0.02] flex items-center justify-center border border-white/10 shadow-2xl relative group overflow-hidden">
                ...
                <ShieldCheck className="w-12 h-12 text-primary/40 group-hover:text-primary transition-colors relative z-10 animate-pulse" />
            </div>
            <div className="space-y-4 relative z-10">
                <div className="space-y-1">
                    <h3 className="text-4xl font-black tracking-tighter text-white uppercase drop-shadow-2xl">
                        {title}
                    </h3>
                    <div className="h-0.5 w-full bg-gradient-to-r from-transparent via-primary to-transparent opacity-50" />
                </div>
                <div className="flex flex-col items-center gap-2">
                    <p className="text-[10px] font-mono text-primary/60 tracking-[0.4em] font-bold uppercase animate-pulse">
                        Access_Restricted // Module_Decryption_In_Progress
                    </p>
                    <p className="text-[9px] font-mono text-muted-foreground/40 max-w-md uppercase tracking-wider leading-relaxed">
                        This strategic domain is currently undergoing deep-neural auditing. Complete the relevant risk-clearance protocols to grant immediate access.
                    </p>
                </div>
            </div>
            {/* Subtle floating elements */}
            <div className="absolute top-1/4 left-1/4 w-1 h-1 bg-primary rounded-full animate-ping" />
            <div className="absolute bottom-1/4 right-1/4 w-1 h-1 bg-indigo rounded-full animate-ping [animation-delay:1s]" />
        </div>
    );
}
            ...
            <main className="flex-1 flex flex-col min-w-0 relative overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar relative z-10 px-6 py-6 space-y-8">
                    ...
                    <div className="grid gap-6 animate-fade-in [animation-delay:500ms] opacity-0 fill-mode-forwards pb-10">
                        ...
                        {/* Restricted Views */}
                        {currentView === 'strategies' && (user?.role === 'Admin' ? <StrategyView /> : <ViewPlaceholder title="STRATEGIES" />)}
                        {currentView === 'correlations' && (user?.role === 'Admin' ? <CorrelationsView /> : <ViewPlaceholder title="CORRELATIONS" />)}
                        {currentView === 'quant' && (user?.role === 'Admin' ? <StrategyBuilder /> : <ViewPlaceholder title="QUANT ENGINE" />)}
                        {currentView === 'alpha' && (user?.role === 'Admin' ? <AlphaScanner /> : <ViewPlaceholder title="ALPHA SCANNER" />)}
                        {currentView === 'intelligence' && (user?.role === 'Admin' ? <IntelligenceView /> : <ViewPlaceholder title="INTELLIGENCE" />)}
                        {currentView === 'security' && (user?.role === 'Admin' ? <SecurityView /> : <ViewPlaceholder title="SECURITY" />)}
