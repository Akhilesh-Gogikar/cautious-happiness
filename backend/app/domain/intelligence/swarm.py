...
from app.models import Source, StructuredAnalysisResult, MirrorAnalysis, NoiseAnalysis, DivergenceAnalysis, AlgoAnalysis
...
class IntelligenceDirectorate:
    def __init__(self):
        self.agents = [
            GeopoliticalAgent(),
            SupplyChainAgent(),
            DerivativesFlowAgent()
        ]

    async def run_swarm(self, question: str, sources: List[Source]) -> Tuple[float, str, StructuredAnalysisResult]:
        # 1. Parallel Analysis
        tasks = [agent.analyze(question, sources) for agent in self.agents]
        agent_reports = await asyncio.gather(*tasks)
        
        # 2. Structured Debate & Synthesis (The "Directorate" Meeting)
        debate_context = "\n\n".join([f"--- {agent.name} Report ---\n{report}" for agent, report in zip(self.agents, agent_reports)])
...
"""
        ...
        collaborative_reasoning = (
            f"Directorate Consensus Strategy:\n"
